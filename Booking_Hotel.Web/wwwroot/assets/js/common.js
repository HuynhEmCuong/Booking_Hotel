...
$(document).ready(function () {
    ...
    function formSuccess() {
        ...
        submitMSG(true, "Xin cảm ơn! Chúng tôi đã nhận được yêu cầu của bạn.");
    }
    ...
    /*-----------------------------------------------------------------
      33. Reservation form
    -------------------------------------------------------------------*/
    $("#reservation-form").validator({ focus: false }).on("submit", function (event) {
        if (event.isDefaultPrevented()) {
            formrRservationError();
    ...
    function submitReservationForm() {
        ...
        //    kids = $("#person-kids").val();
        //     window.location = "/dat-phong" + "&checkInDate=" + convertVnDateToEnDate(checkIn) + "&checkOutDate=" + convertVnDateToEnDate(checkOut) + "&roomCatId=" + roomCatId + "&adult=" + adult + "&kids=" + kids;
        //    //$.ajax({
            //    type: "POST",
            //    url: "/dat-phong",
            //    data: "&checkInDate=" + convertVnDateToEnDate(checkIn) + "&checkOutDate=" + convertVnDateToEnDate(checkOut) + "&roomCatId=" + roomCatId + "&adult=" + adult + "&kids=" + kids,
            //    success: function (text) {
            //        if (text == "success") {
            //            formRservationSuccess();
            //        } else {
            //            formrRservationError();
            //            submitRservationMSG(false, text);
            //        }
            //    }
            //});
    }

    function formRservationSuccess() {
        $("#reservation-form")[0].reset();
        submitMSG(true, "Xin cảm ơn! Chúng tôi đã nhận được yêu cầu của bạn.");
    }

    function formrRservationError() {

    }

    function submitRservationMSG(valid, msg) {
        if (valid) {
            var msgClasses = "validation-success";
        } else {
            var msgClasses = "validation-danger";
    ...
    function convertVnDateToEnDate(date) {
        if (date === null || date === undefined || date === '') {
            return "";
        } else {
            let enmonth = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
            let vnmonth = [ "Th1", "Th2", "Th3", "Th4", "Th5", "Th6", "Th7", "Th8", "Th9", "Th10","Th11", "Th12"];
            vnmonth.forEach(function (value, i) {
                if (date.includes(value)) {
                    date = date.replace(value, enmonth[i]);
                    return;
                }
            });
            return date;
